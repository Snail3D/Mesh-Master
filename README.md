...
- **Per-Channel Message Tracking** — Activity dashboard now shows message counts for each configured channel with 30-day historical charts on hover. Channel names auto-populate from radio settings (e.g., "LongFast", "YourNet").
...
If MESH MASTER is connected to multiple networks (e.g., YourNet + MainChannel), it acts as a bridge:
- Users on YourNet can relay to users on MainChannel and vice versa
